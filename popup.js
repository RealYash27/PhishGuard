...
async function geoFromIp(ip) {
  // 1. ipwho.is — no ?fields= param (it suppresses fields for CDN IPs)
  try {
    const j = await (await fetch(`https://ipwho.is/${ip}`)).json();
    if (j && j.success === true && j.country_code) {
      return { cc: String(j.country_code).toUpperCase(), country: j.country ? String(j.country) : "" };
    }
  } catch {}
  // 2. ipapi.co — skip if throttled
  try {
    const j = await (await fetch(`https://ipapi.co/${ip}/json/`)).json();
    if (j && !j.error && j.country_code) {
      return { cc: String(j.country_code).toUpperCase(), country: j.country_name ? String(j.country_name) : "" };
    }
  } catch {}
  // 3. ipinfo.io — most reliable free fallback (50k req/month)
  try {
    const j = await (await fetch(`https://ipinfo.io/${ip}/json`)).json();
    if (j && j.country && !j.error) {
      const cc = String(j.country).toUpperCase();
      return { cc, country: CC_TO_NAME[cc] || cc };
    }
  } catch {}
  return null;
}
// Country code → full name (used when API only returns a 2-letter code)
const CC_TO_NAME = {
  US:"United States",GB:"United Kingdom",IN:"India",DE:"Germany",FR:"France",
  CN:"China",JP:"Japan",RU:"Russia",BR:"Brazil",CA:"Canada",AU:"Australia",
  KR:"South Korea",NL:"Netherlands",SG:"Singapore",SE:"Sweden",NO:"Norway",
  CH:"Switzerland",IT:"Italy",ES:"Spain",PL:"Poland",MX:"Mexico",ID:"Indonesia",
  NG:"Nigeria",ZA:"South Africa",PK:"Pakistan",BD:"Bangladesh",TR:"Turkey",
  UA:"Ukraine",AR:"Argentina",NZ:"New Zealand",PT:"Portugal",BE:"Belgium",
  AT:"Austria",DK:"Denmark",FI:"Finland",HK:"Hong Kong",TW:"Taiwan",
  TH:"Thailand",MY:"Malaysia",PH:"Philippines",VN:"Vietnam",AE:"UAE",
  SA:"Saudi Arabia",EG:"Egypt",IL:"Israel",IR:"Iran",CZ:"Czech Republic",
  RO:"Romania",HU:"Hungary",GR:"Greece",CO:"Colombia",CL:"Chile",
};
function getCountryFromTld(host) {
  const tld = (host.split(".").pop() || "").toLowerCase();
  const map = {
    in:"IN", us:"US", uk:"GB", au:"AU", ca:"CA", de:"DE", fr:"FR", jp:"JP",
    cn:"CN", ru:"RU", br:"BR", kr:"KR", nl:"NL", sg:"SG", se:"SE", no:"NO",
    ch:"CH", it:"IT", es:"ES", pl:"PL", mx:"MX", id:"ID", ng:"NG", za:"ZA",
    pk:"PK", bd:"BD", tr:"TR", ua:"UA", ar:"AR", nz:"NZ", pt:"PT", be:"BE",
    at:"AT", dk:"DK", fi:"FI", hk:"HK", tw:"TW", th:"TH", my:"MY", ph:"PH",
    vn:"VN", ae:"AE", sa:"SA", eg:"EG", il:"IL", ir:"IR", cz:"CZ", ro:"RO",
  };
  return map[tld] || "";
}
...
function renderFromResponse(res) {
  ...
  const opHit = sources.some(s => s.includes("openphish"));
  const abHit = sources.some(s => s.includes("abuseipdb"));
  ...
  setProviderStatus("opStatus", "opNote",
    opHit ? "Flagged" : "No hit",
    opHit ? (find("openphish") || "Found in OpenPhish live feed") : "No match");
  
  const abReason = find("abuseipdb");
  setProviderStatus("abStatus", "abNote",
    abHit ? "Flagged" : (abReason ? "Clean" : "No hit"),
    abReason || "No match");
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // ---------- Pending downloads panel ----------
  function renderPendingDownloads(pending) {
    const panel = $("pendingDlPanel");
    const list  = $("pendingDlList");
    ...
    list.innerHTML = pending.map(p => {
      const name    = escapeHtml(p.filename || "Unknown file");
      const reasons = (p.reasons || []).join(" · ") || "Suspicious signals detected";
      const sevTag  = p.severity === "red"
        ? "<span class='badge red'>High risk</span>"
        : "<span class='badge yellow'>Suspicious</span>";
      return `
        <div class="pdl-item" data-notif="${escapeHtml(p.notifId)}">
          <div class="pdl-name">${sevTag} ${name}</div>
          <div class="pdl-reasons">${escapeHtml(reasons)}</div>
          <div class="pdl-btns">
            <button class="pdl-allow">✔ Allow</button>
            <button class="pdl-cancel">✖ Cancel</button>
          </div>
        </div>`;
    }).join("");

    list.querySelectorAll(".pdl-item").forEach(item => {
      const notifId = item.dataset.notif;
      item.querySelector(".pdl-allow").onclick = () => {
        chrome.runtime.sendMessage({ type: "DOWNLOAD_DECIDE", notifId, allow: true });
        item.remove();
        if (!list.querySelector(".pdl-item")) panel.style.display = "none";
      };
      item.querySelector(".pdl-cancel").onclick = () => {
        chrome.runtime.sendMessage({ type: "DOWNLOAD_DECIDE", notifId, allow: false });
        item.remove();
        if (!list.querySelector(".pdl-item")) panel.style.display = "none";
      };
    });
  ...
  chrome.runtime.sendMessage({ type: "PENDING_DL_GET" }, res => {
    renderPendingDownloads(res?.pending || []);
  });
